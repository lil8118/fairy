// open a small modal to pick a student and give the selected task
          ...
          if(userId){
            ...
            try{
                ...
                try{
                  for(let i=0;i<localStorage.length;i++){
                    ...
                    try{
                      ...
                      try{ const sessLog = await supClient.auth.getSession().catch(()=>null); console.log('syncAllLocalToRemote upsert attempt', { userId: uid, session: sessLog && sessLog.data && sessLog.data.session ? sessLog.data.session.user?.id : null, payloadOwner: payload.owner, id: payload.id }); }catch(e){}
              ...
              // Only run automatic sync if explicitly enabled. Default: disabled.
               ...
               // show a small non-blocking banner so the user can enable sync or copy the SQL fix
                try{
                  if(!document.getElementById('lf-sync-banner')){
                    const b = document.createElement('div'); b.id = 'lf-sync-banner';
                    b.style.cssText = 'position:fixed;left:16px;right:16px;bottom:18px;background:linear-gradient(90deg,#fff8fb,#f0fbff);border:1px solid rgba(11,18,32,0.06);padding:12px 14px;border-radius:12px;z-index:99999;display:flex;align-items:center;gap:12px;box-shadow:0 12px 36px rgba(10,12,30,0.06);font-size:14px';
                    b.innerHTML = `<div style="flex:1"><strong>Sync paused</strong><div style='color:#556;margin-top:4px'>Automatic sync is disabled because recent server writes were blocked. Enable after fixing Supabase RLS policies.</div></div>`;
                    const showSql = document.createElement('button'); showSql.className='small-btn'; showSql.textContent = 'Show Fix SQL';
                    const enableBtn = document.createElement('button'); enableBtn.className='action-btn'; enableBtn.textContent = 'Enable Sync';
                    const blockedBtn = document.createElement('button'); blockedBtn.className = 'small-btn'; blockedBtn.textContent = 'Blocked items';
                    const dismiss = document.createElement('button'); dismiss.className='btn-muted'; dismiss.textContent = 'Dismiss';
                    b.appendChild(showSql); b.appendChild(enableBtn); b.appendChild(dismiss);
                    // expose a quick entry to show blocked sync items
                    try{ blockedBtn.addEventListener('click', ()=>{ try{ window._lf_showBlockedSyncModal && window._lf_showBlockedSyncModal(); }catch(e){} }); b.appendChild(blockedBtn); }catch(e){}
                    document.body.appendChild(b);
                    const fixSQL = "-- Run in Supabase SQL editor (owner column is uuid)\nALTER TABLE public.classrooms ENABLE ROW LEVEL SECURITY;\nCREATE POLICY \"classrooms_select_policy\" ON public.classrooms FOR SELECT TO authenticated USING ( owner = auth.uid() );\nCREATE POLICY \"classrooms_insert_policy\" ON public.classrooms FOR INSERT TO authenticated WITH CHECK ( owner = auth.uid() );\nCREATE POLICY \"classrooms_update_policy\" ON public.classrooms FOR UPDATE TO authenticated USING ( owner = auth.uid() ) WITH CHECK ( owner = auth.uid() );\nCREATE POLICY \"classrooms_delete_policy\" ON public.classrooms FOR DELETE TO authenticated USING ( owner = auth.uid() );";
                    showSql.addEventListener('click', ()=>{
                      const mb = document.createElement('div'); mb.className='modal-backdrop'; mb.style.cssText='position:fixed;inset:0;background:rgba(10,12,20,0.45);display:flex;align-items:center;justify-content:center;z-index:100000';
                      const inner = document.createElement('div'); inner.style.cssText='background:#fff;padding:14px;border-radius:10px;max-width:760px;max-height:70vh;overflow:auto;font-size:13px';
                      inner.innerHTML = `<h3 style='margin:0 0 8px 0'>Supabase RLS Fix SQL</h3><pre style='white-space:pre-wrap;background:#f7f9fc;padding:12px;border-radius:8px;overflow:auto;'>${fixSQL}</pre><div style='text-align:right;margin-top:12px'><button id='copySql' class='action-btn'>Copy</button><button id='closeSql' class='btn-muted' style='margin-left:8px'>Close</button></div>`;
                      mb.appendChild(inner); document.body.appendChild(mb);
                      mb.querySelector('#copySql').addEventListener('click', ()=>{ try{ navigator.clipboard.writeText(fixSQL); if(window.showToast) window.showToast('SQL copied to clipboard'); }catch(e){ if(window.showToast) window.showToast('Copy failed'); } });
                      mb.querySelector('#closeSql').addEventListener('click', ()=>{ try{ mb.remove(); }catch(e){} });
                    });
                    enableBtn.addEventListener('click', async ()=>{
                      try{ window._lf_enable_sync = true; enableBtn.disabled = true; enableBtn.textContent = 'Enabling…'; await syncAllLocalToRemote(sup, userId); if(window.showToast) window.showToast('Manual sync attempted — check console for details'); b.remove(); }catch(e){ console.warn(e); if(window.showToast) window.showToast('Sync attempt failed — check console'); enableBtn.disabled = false; enableBtn.textContent = 'Enable Sync'; }
                    });
                    // Expose the callable sync function for debugging from the console
                    try{ window._lf_syncAllLocalToRemote = async function(){ try{ const sClient = window._lf_supabase || sup; const sess = await sClient.auth.getSession().catch(()=>null); const uid = (window._lf_current_user_id) ? window._lf_current_user_id : (sess && sess.data && (sess.data.session?.user?.id || sess.data.user?.id) ? (sess.data.session?.user?.id || sess.data.user?.id) : null); return await syncAllLocalToRemote(sClient, uid); }catch(e){ console.warn('window._lf_syncAllLocalToRemote failed', e); throw e; } }; }catch(e){}
                    // Add a small  button into topbarActions for convenience
                    try{
                      const topbarActionsEl = document.getElementById('topbarActions');
                      if(topbarActionsEl && !document.getElementById('manualSyncBtn')){
                        const syncBtn = document.createElement('button'); syncBtn.id = 'manualSyncBtn'; syncBtn.className = 'small-btn'; syncBtn.textContent = ;
                        syncBtn.addEventListener('click', async ()=>{
                          try{ syncBtn.disabled = true; const prev = syncBtn.textContent; syncBtn.textContent = 'Syncing…'; await window._lf_syncAllLocalToRemote(); if(window.showToast) window.showToast('Sync complete', {type:'success'}); syncBtn.textContent = prev; syncBtn.disabled = false; }
                          catch(e){ console.warn('manual sync failed', e); if(window.showToast) window.showToast('Sync failed — check console', {type:'error'}); syncBtn.textContent = ; syncBtn.disabled = false; }
                        });
                        topbarActionsEl.appendChild(syncBtn);
                        // also add quick access to blocked items from the topbar
                        try{ if(!document.getElementById('manualBlockedBtn')){ const mb = document.createElement('button'); mb.id = 'manualBlockedBtn'; mb.className = 'small-btn'; mb.textContent = 'Blocked'; mb.addEventListener('click', ()=>{ try{ window._lf_showBlockedSyncModal && window._lf_showBlockedSyncModal(); }catch(e){} }); topbarActionsEl.appendChild(mb); } }catch(e){}
                      }
                    }catch(e){}
                    dismiss.addEventListener('click', ()=>{ try{ b.remove(); }catch(e){} });
                  }
                }catch(e){ console.warn('failed to create sync banner', e); }
              }
            }catch(e){ console.warn('syncAllLocalToRemote failed', e); }
    ...
    async function saveClassroom(id, snapshot){
      ...
      try{
        ...
        function addStudent(s){ if(!s || !s.id) return; if(!studentsMap.has(s.id)) studentsMap.set(s.id, JSON.parse(JSON.stringify(s))); else {
        ...
        try{ const sessLog = await sup.auth.getSession().catch(()=>null); console.log('saveClassroom upsert', { userId: userId, session: sessLog && sessLog.data && sessLog.data.session ? sessLog.data.session.user?.id : null, id }); }catch(e){}
        ...
        if(upsertRes && upsertRes.error){
          ...
          try{ if(upsertRes.error && upsertRes.error.code === '42501'){ try{ window.showToast && window.showToast('Sync blocked by DB policy (Row-Level Security). Check Supabase policies for table `classrooms`.', {type:'error', duration:8000}); }catch(e){} }
          }catch(e){}
  ...
  if(deleteBtn){ deleteBtn.addEventListener('click', ()=>{ if(!selectedClassId){ showToast('Select a class first', {type:'warning'}); return; } openConfirm('Delete this class? This will remove all students and cannot be undone.').then(ok=>{ if(ok) removeClassroomById(selectedClassId); }); }); }
    ...
    async function openAddStudentModal(classId, studentId=null){
      let snap = loadClassroom(classId);
      if(!snap){ try{ showToast('Loading class…', {type:'default'}); snap = await ensureClassLoaded(classId); }catch(e){}
        if(!snap){ showToast('Failed to load class', {type:'error'}); return; }
      }
      ...
      clearPhotoBtn.onclick = ()=>{ previewImg.dataset.uploaded=''; mb.dataset.photoSource = 'avatar'; // reset to default avatar
    ...
    async function openAddEditTasksModal(classId){
      ...
      function renderMgr(){ mgr.innerHTML = ''; (snap.tasks||[]).forEach((t,i)=>{
    ...
    function openAddEditStudentsModal(classId){
      ...
      function renderMgr(){ mgrArea.innerHTML = ''; (snap.students||[]).forEach((s, idx)=>{
        ...
        del.onclick = ()=>{ openConfirm('Delete '+s.name+'? This cannot be undone.', {confirmLabel:'Delete', cancelLabel:'Cancel'}).then(ok=>{ if(!ok) return; snap.students.splice(idx,1); saveClassroom(classId, snap); renderMgr(); try{ openClass(classId); }catch(e){} if(editingId === s.id){ // cleared editing
